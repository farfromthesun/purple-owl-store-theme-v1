...
function productGetPickedVariant(e) {
  ...
  const allVariants = JSON.parse(optionsContainer.querySelector("script").textContent);
...
function productChangeATCStatus(disable) {
  ...
  if (!atcButton) return;
  disable ? atcButton.setAttribute("disabled", "disabled") : atcButton.removeAttribute("disabled");
}
...
async function productOptionsChangeUpdateInfo(e, variantID) {
  ...
  const productBuyToRender = productInfoContainerToRender.querySelector(".product-buy");
...
function init() {
  ...
  if (_body.dataset.template == "product") {
    ...
    document.querySelector(".product-quantity-input").addEventListener("change", function (e) {
