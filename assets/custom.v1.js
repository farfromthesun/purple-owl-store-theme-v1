...
async function fetchShopifySection(url) {
  ...
  shopifySectionsCache = [...shopifySectionsCache, {
    responseText,
    url
  }];
  return responseText;
...
async function productsGridRenderSections(e, searchParams) {
  ...
  const htmlText = shopifySectionsCache.some(element => element.url === url) ? shopifySectionsCache.find(element => element.url === url).responseText : await fetchShopifySection(url);
...
function productChangeATCStatus(disable) {
  let text = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
function productOptionsChangeUpdateUrl(e, variantID) {
  window.history.replaceState({}, "", `${e.currentTarget.dataset.url}?variant=${variantID}`);
}
function allProductVariantsJSON() {
  const allVariants = JSON.parse(document.querySelector(".product-options").querySelector("script").textContent);
  return allVariants;
}
async function productOptionsChangeUpdateInfo(e, variantID, variantFeaturedMedia) {
  ...
  const productQuantityTitle = productInfoContainer.querySelector(".product-quantity-title-container");
  ...
  htmlText = await fetchShopifySection(url);
  const htmlToRender = new DOMParser().parseFromString(htmlText, "text/html");
  const productInfoContainerToRender = htmlToRender.querySelector(".product-info");
  const productPricesToRender = productInfoContainerToRender.querySelector(".product-prices");
  const productOptionsInnerToRender = productInfoContainerToRender.querySelector(".product-options-inner");
  const productQuantityTitleToRender = productInfoContainerToRender.querySelector(".product-quantity-title-container");
  ...
  if (productImages.firstElementChild.classList[0] !== productImagesToRender.firstElementChild.classList[0] || variantFeaturedMedia) {
...
function productOptionsChangeHandler(e) {
  ...
  productOptionsChangeUpdateInfo(e, pickedVariant.id, pickedVariant.featured_media);
}
function productGlideInit() {
  if (document.querySelector(".product-gallery")) {
    var glide = new _glidejs_glide__WEBPACK_IMPORTED_MODULE_1__["default"](".glide");
    glide.on("move.after", function () {
      document.querySelector(".product-gallery").querySelector(".slides-status-current").innerHTML = glide.index + 1;
    });
    glide.mount();
  }
}
...
function productAddToCartErrorsHandler() {
  let errorDescription = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  const errorContainer = document.querySelector(".product-main-inner").querySelector(".product-buy").querySelector(".product-buy-error-container");
  if (!errorContainer) return;
  if (errorDescription) {
    errorContainer.classList.add("show");
    errorContainer.querySelector(".product-buy-error-message").textContent = errorDescription;
  } else {
...
function cartBasicSectionsToUpdate() {
  ...
  const basicSectionsToUpdateNames = sectionsToUpdate.map(section => section.sectionName);
  return {
...
async function cartDrawerClear(e) {
  ...
  try {
    ...
    const responseJSON = await response.json();
    sectionsToUpdate.forEach(section => {
      const htmlToInject = new DOMParser().parseFromString(responseJSON.sections[section.sectionName], "text/html").getElementById(section.htmlId);
      document.getElementById(section.htmlId).innerHTML = htmlToInject.innerHTML;
    });
    cartDrawerInner.classList.add("is-empty");
  } catch (error) {
...
function init() {
  ...
  if (isPageCart) {
    ...
    document.querySelector(".apply-discount-code").addEventListener("click", function (e) {
      applyCoupon(e);
    });
  }
}
